...
try_install_rocksdb() {
    if [ ! -f rocksdb.deb ]; then
        ...
        if [ "$arch" == "aarch64" ]; then
            echo "Host arch is aarch64"
            wget -O rocksdb.deb https://github.com/lizhanhui/rocksdb/releases/download/rocksdb-v8.1.3/rocksdb-aarch64.deb
        elif [ "$arch" == "x86_64" ]; then
            echo "Host arch is amd64"
            wget -O rocksdb.deb https://github.com/lizhanhui/rocksdb/releases/download/rocksdb-v8.1.3/rocksdb.deb
        else
            echo "Unsupported arch"
            exit 1
        fi
